...
const Home = () => {
    ...
    useEffect(() => {
        ...
        const tarotCard = cardNumberChanger({ number: drowCard.stackingOrder, position: drowCard.position })
        setDrowCardValue(`${tarotCard.number} ${tarotCard.name}の${tarotCard.position}`)
        setDrowCardName(`${tarotCard.number} ${tarotCard.name}`)
    }, [drowCard])

    const [infomationText, setInfomationText] = useState<string | Array<string>>()
    ...
    const [tarotCardInfomationText, setTarotCardInfomationText] = useState<string | Array<string>>()
    const handleTarotCardInfomation = () => {
        if (drowCard === undefined) return
        const infomation = tarotInfomation({ infoType: 'card', cardNumber: drowCard.stackingOrder })
        if (infomation === undefined) return
        ...
        <>
            ...
            <Flex>
                ...
                <Box ml={2}>
                    <Popover>
                        <PopoverTrigger>
                            <Badge ml="1" mt="6" colorScheme="blue" onClick={() => handleInfomation()}>
                                ?
                            </Badge>
                        </PopoverTrigger>
                        <PopoverContent>
            ...
            <Box ml={1}>
                <Button onClick={() => drow()} isDisabled={isDrowDisabled || drowCardIndex > 21}>
                    ドロー
                </Button>
                <Flex>
                    ...
                    {drowCard && (
                        <Box ml={2}>
                            <Popover>
                                <PopoverTrigger>
                                    <Badge ml="1" mt="1" colorScheme="blue" onClick={() => handleInfomation()}>
                                        ?
                                    </Badge>
                                </PopoverTrigger>
                                <PopoverContent>
